 ...
 *
 */
class AliyunBailianTranslator : LanguageGroupedTranslator() {
    ...
    companion object {
        const val NAME = "AliyunBailian"
    }

    private var url: String = "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("AliyunBailianTranslator init failed, url is invalid: $url")
            return false
        }
        ...
        apiKey = if (fileApiKey.isNullOrBlank()) {
            (System.getenv("ALIYUN_BAILIAN_API_KEY") ?: "").trim()
        } else {
            fileApiKey.trim()
        }

        if (apiKey.isBlank()) {
            logger.error("AliyunBailianTranslator init failed, Aliyun Bailian apiKey not found. Please set apiKey in AliyunBailian.yaml or ALIYUN_BAILIAN_API_KEY in environment.")
            return false
        }
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("AliyunBailianTranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("AliyunBailianTranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("AliyunBailianTranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("AliyunBailianTranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")

            val begin = System.currentTimeMillis()
            val body: RequestBody = payload.toRequestBody("application/json; charset=utf-8".toMediaType())

            val builder = Request.Builder()
            builder.url(url).addHeader("Authorization", "Bearer $apiKey")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("AliyunBailianTranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("AliyunBailianTranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("AliyunBailianTranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "AliyunBailianTranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
