 ...
 *
 */
class BigModelTranslator : LanguageGroupedTranslator() {
    ...
    companion object {
        const val NAME = "BigModel"
    }

    private var url: String = "https://open.bigmodel.cn/api/paas/v4/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("BigModelTranslator init failed, url is invalid: $url")
            return false
        }
        ...
        apiKey = if (fileApiKey.isNullOrBlank()) {
            (System.getenv("BIGMODEL_API_KEY") ?: "").trim()
        } else {
            fileApiKey.trim()
        }

        if (apiKey.isBlank()) {
            logger.error("BigModelTranslator init failed, BigModel apiKey not found. Please set apiKey in BigModel.yaml or BIGMODEL_API_KEY in environment.")
            return false
        }
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("BigModelTranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("BigModelTranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("BigModelTranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("BigModelTranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("BigModelTranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("BigModelTranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("BigModelTranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "BigModelTranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
