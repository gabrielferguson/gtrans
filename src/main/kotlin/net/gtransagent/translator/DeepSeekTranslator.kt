 ...
 *
 */
class DeepSeekTranslator : LanguageGroupedTranslator() {
    ...
    companion object {
        const val NAME = "DeepSeek"
    }

    private var url: String = "https://api.deepseek.com/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("DeepSeekTranslator init failed, url is invalid: $url")
            return false
        }
        ...
        if (apiKey.isBlank()) {
            logger.error("DeepSeekTranslator init failed, DeepSeek apiKey not found. Please set apiKey in DeepSeek.yaml or DEEPSEEK_API_KEY in environment.")
            return false
        }
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("DeepSeekTranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("DeepSeekTranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("DeepSeekTranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("DeepSeekTranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("DeepSeekTranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("DeepSeekTranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("DeepSeekTranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "DeepSeekTranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
