 ...
 *
 */
class MistralTranslator : LanguageGroupedTranslator() {
    ...
    companion object {
        const val NAME = "Mistral"
    }

    private var url: String = "https://api.mistral.ai/v1/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("MistralTranslator init failed, url is invalid: $url")
            return false
        }
        ...
        apiKey = if (fileApiKey.isNullOrBlank()) {
            (System.getenv("MISTRAL_API_KEY") ?: "").trim()
        } else {
            fileApiKey.trim()
        }

        if (apiKey.isBlank()) {
            logger.error("MistralTranslator init failed, Mistral apiKey not found. Please set apiKey in Mistral.yaml or MISTRAL_API_KEY in environment.")
            return false
        }
        ...
        try {
            ...
            logger.error("MistralTranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("MistralTranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("MistralTranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("MistralTranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("MistralTranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("MistralTranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "MistralTranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
