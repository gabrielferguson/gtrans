 ...
 *
 */
class OpenAITranslator : LanguageGroupedTranslator() {
    ...
    private var url: String = "https://api.openai.com/v1/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("OpenAITranslator init failed, url is invalid: $url")
            return false
        }
        ...
        if (apiKey.isBlank()) {
            logger.error("OpenAITranslator init failed, OpenAI apiKey not found. Please set apiKey in OpenAI.yaml or OPENAI_API_KEY in environment.")
            return false
        }
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("OpenAITranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("OpenAITranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("OpenAITranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("OpenAITranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("OpenAITranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("OpenAITranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("OpenAITranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "OpenAITranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
