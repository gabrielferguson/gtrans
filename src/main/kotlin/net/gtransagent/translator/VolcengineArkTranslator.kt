 ...
 *
 */
class VolcengineArkTranslator : LanguageGroupedTranslator() {
    ...
    companion object {
        const val NAME = "VolcengineArk"
    }

    private var url: String = "https://ark.cn-beijing.volces.com/api/v3/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("VolcengineArkTranslator init failed, url is invalid: $url")
            return false
        }
        ...
        apiKey = if (fileApiKey.isNullOrBlank()) {
            (System.getenv("VOLCENGINE_ARK_API_KEY") ?: "").trim()
        } else {
            fileApiKey.trim()
        }

        if (apiKey.isBlank()) {
            logger.error("VolcengineArkTranslator init failed, Volcengine Ark apiKey not found. Please set apiKey in VolcengineArk.yaml or VOLCENGINE_ARK_API_KEY in environment.")
            return false
        }
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("VolcengineArkTranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("VolcengineArkTranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("VolcengineArkTranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("VolcengineArkTranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("VolcengineArkTranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("VolcengineArkTranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("VolcengineArkTranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "VolcengineArkTranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
