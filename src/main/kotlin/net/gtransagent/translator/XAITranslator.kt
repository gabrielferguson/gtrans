 ...
 *
 */
class XAITranslator : LanguageGroupedTranslator() {
    ...
    private var url: String = "https://api.x.ai/v1/chat/completions"
    ...
    override fun init(configs: Map<*, *>): Boolean {
        ...
        try {
            ...
            logger.error("XAITranslator init failed, url is invalid: $url")
            return false
        }
        ...
        apiKey = if (fileApiKey.isNullOrBlank()) {
            (System.getenv("XAI_API_KEY") ?: "").trim()
        } else {
            fileApiKey.trim()
        }

        if (apiKey.isBlank()) {
            logger.error("XAITranslator init failed, xAI apiKey not found. Please set apiKey in XAI.yaml or XAI_API_KEY in environment.")
            return false
        }
        ...
        if (configs["engineMapping"] == null || (configs["engineMapping"] as Map<String, Map<String, String>>).isEmpty()) {
            logger.error("XAITranslator init failed, engineMapping is null or empty")
            return false
        }
        ...
        try {
            ...
            logger.error("XAITranslator init failed, engineMapping is invalid: $e")
            return false
        }

        logger.info("XAITranslator init success, url: $url, supportedEngines: $supportedEngines")
        return true
    }
    ...
    override fun sendRequest(
        ...
        try {
            ...
            logger.info("XAITranslator translateTexts begin, payload: $payload, src: $sourceLang, target: $targetLang, engine:$engineCode")
            ...
            if (response.isSuccessful && Objects.nonNull(response.body)) {
                ...
                logger.info("XAITranslator translateTexts end, time: ${end - begin} ms, result: ${results}, input: ${inputs}, src: ${sourceLang}, target: ${targetLang}, engine:${engineCode}")
                return results
            } else {
                logger.error("XAITranslator return code invalid,  input:${inputs}, code:${response.code}")
                throw Status.UNAVAILABLE.withDescription("XAITranslator return code invalid, code:${response.code}")
                    .asRuntimeException()
            }
        } catch (e: Exception) {
            logger.warn(
                "XAITranslator failure, input:${inputs}, error:${e}", e
            )
            throw e
        }
    }
